...
import { getCkbTransferSummaries, getSudtTransferSummaries, TransferDirection, TransferSummary } from '../api'
  ...
  return (
    <TransactionListItemWrapper onClick={() => push(`${match.url}/transactions/${tx.txHash}`)}>
      <Row gutter={8} align="middle">
        <Col flex="40px">
          <TransactionStatusIcon status={tx.status} direction={tx.direction} />
        </Col>
        <Col flex="auto">
          <Row gutter={8}>
            <Col flex="auto">
          ...
          <Row justify="end" className="amount">
            <span className="symbol">{tx.direction === 'in' ? '+' : '-'}</span>
            <Balance value={tx.amount} type={tx.tokenName} />
          </Row>
        </Col>
      </Row>
...
export const TransactionList: React.FC<TransactionListProps> = (/* props: TransactionListProps */) => {
  ...
  const [transferDirection, setTransferDirectionFilter] = useState<'all' | TransferDirection>('all')
  const { tokenName } = useParams<{ tokenName: string }>()

  const { data: txs, isLoading } = useQuery<TransferSummary[]>(['transactions', tokenName, transferDirection], () => {
    if (tokenName === 'CKB') return getCkbTransferSummaries()
  ...
  function onDirectionChange(direction: string) {
    setTransferDirectionFilter(direction as 'all' | TransferDirection)
  }

  if (isLoading) return <Spin size="large" tip="loading..." />
  const transferList =
    txs && txs.length ? (
      txs.map((tx: TransferSummary) => <TransactionListItem key={tx.txHash} transaction={tx} />)
    ) : (
